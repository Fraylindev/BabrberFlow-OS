...
Actualizado: 2026-09-03. Este documento describe el producto y el estado actual. El historial completo anterior a G0 se preserva en [`docs/history/PROJECT_MASTER_LEGACY_2026-08-13.md`](docs/history/PROJECT_MASTER_LEGACY_2026-08-13.md).
...
| 4 | Servicios — A.1 Backend: ordenamiento | **CERRADO / APROBADO** | Aprobación explícita del propietario sobre `9752842dfc8a3a53cdcb6bc06e61a185bc79a385` |
| 4 | Servicios — Entrega B Frontend | **CERRADO / APROBADO** | Aprobación oficial del propietario 2026-09-02 sobre `79706ffdc16e9225e6e6528845c9e445a7829ff0` tras QA manual |
| 4 | Módulo Servicios | **CERRADO / APROBADO** | Backend A/A.1 y Frontend B aprobados; siguiente paso limitado a auditoría de Equipo |
...
| 6 | Equipo — Entrega B Frontend | **IMPLEMENTADO / EN REVISIÓN** | Directorio, invitaciones y gestión de accesos sobre los contratos aprobados; pendiente de QA/aprobación del propietario |
| 7–8 | Configuración, Analytics modular | **PENDIENTES** | No iniciar sin autorización propia |
...
Servicios — A.1 Backend de ordenamiento: **CERRADO / APROBADO** por decisión explícita del propietario sobre `9752842dfc8a3a53cdcb6bc06e61a185bc79a385`. `GET /services` conserva el filtro independiente `isActive` y añade `sort` para nombre, reservas no canceladas, antigüedad y precio. El valor omitido mantiene `name ASC, id ASC`; todos los órdenes tienen desempate estable y permanecen tenant-scoped. Los conteos solo intervienen en el orden, no se exponen, y la respuesta continúa limitada campo por campo a la proyección aprobada. No hay cambio Prisma ni migración. La evidencia aprobada conserva Prisma validate/generate, TypeScript, lint y build en exit `0`; 392 unitarias API aprobadas/11 omitidas; 56/56 unitarias dirigidas y 15/15 E2E aisladas con 19 migraciones.

Servicios — Entrega A Backend queda **CERRADO / APROBADO** por decisión oficial del propietario el 2026-08-31 sobre `f7088a4abb14e61721f08f7eeb85adbd8e6650d6`; A.1 de ordenamiento queda **CERRADO / APROBADO** sobre `9752842dfc8a3a53cdcb6bc06e61a185bc79a385`. Servicios — Entrega B Frontend y el módulo completo quedan **CERRADOS / APROBADOS** por decisión oficial del propietario el 2026-09-02 sobre `79706ffdc16e9225e6e6528845c9e445a7829ff0`, tras aceptar su QA manual. El frontend consume las proyecciones mínimas publicadas, filtra y ordena en backend mediante controles independientes, y permite listar, crear, editar, desactivar y reactivar. OWNER/ADMIN gestionan; BARBER/RECEPTIONIST reciben una vista de solo lectura. La consulta se aísla por usuario, organización y rol; cambiar de contexto desmonta datos, filtros y modales, y una instancia única por visita impide efectos tardíos incluso en A → B → A. El formulario conserva el foco al escribir, restringe precio a un monto DOP numérico, expresa duración en minutos enteros con equivalencias naturales y reserva un slot no interactivo para una futura imagen sin integrar Cloudinary ni ampliar el contrato. La UI mantiene loading, vacío, error/reintento, pending y éxito, con tabla desktop y tarjetas móviles. La evidencia aceptada conserva Web TypeScript, lint, 35 pruebas, build y QA real con Clerk Development y PostgreSQL desechable para OWNER → BARBER → OWNER, datos y acciones tenant-scoped, combinación de estado/orden, alta y edición sin pérdida de foco, error terminal sin datos obsoletos, reintento, desktop y 375 px sin overflow; consola sin errores de aplicación. No cambia backend, Prisma, Reservas, Facturación, página pública, `ProfessionalService` ni medios. El siguiente paso autorizado se limita a auditar Equipo, sin implementarlo.

Equipo — Entrega A Backend: **CERRADO / APROBADO** por decisión oficial del propietario sobre `1270ce9958b3da78f1d2be27d06545a8546c6d43`. OWNER/ADMIN conservan invitación, listado, reenvío y revocación mediante Clerk; OWNER no es invitable y crear de nuevo una invitación abierta con el mismo correo/rol/perfil reutiliza el registro existente. Las acciones sensibles de invitación tienen límites específicos. El directorio `GET /organizations/mine/team-members` pagina solo nombre, correo, rol, acceso activo y nombre/estado de Professional vinculado; BARBER/RECEPTIONIST no lo reciben. Cambio de rol y revocación usan correo normalizado como selector explícito, revalidan actor y tenant dentro de una transacción `SERIALIZABLE`, bloquean la organización, protegen al último OWNER y prohíben que ADMIN opere sobre OWNER. Ambas mutaciones aplican además un límite específico de 10 solicitudes por minuto mediante el `ThrottlerGuard` existente. La revocación elimina únicamente Membership y preserva User, Professional e historial; las mutaciones reales registran AuditLog transaccional sin PII. `GET /organizations/mine/members` permanece compatible exclusivamente para el vínculo ya cerrado de Profesionales y conserva OWNER/ADMIN, tenant y exclusión de teléfono/secretos; no se reutiliza como directorio de Equipo. No hay migración ni cambio Prisma, Clerk/Auth legacy u otros módulos. La evidencia aprobada conserva TypeScript, lint y build; 404 unitarias aprobadas/11 omitidas, 3/3 E2E del correctivo, 14/14 E2E dirigidas y 121/121 E2E completas sobre PostgreSQL temporal aislado.

Equipo — Entrega B Frontend: **IMPLEMENTADO / EN REVISIÓN**. La pantalla consume exclusivamente los contratos aprobados para mostrar un directorio paginado mínimo y las invitaciones, cambiar roles asignables y revocar accesos con confirmación. Revocar una invitación también exige ahora una confirmación accesible que identifica el correo y explica que el enlace dejará de ser válido; abrir, cancelar o cerrar el diálogo no inicia la petición, que solo se envía desde la confirmación final. OWNER/ADMIN gestionan; ADMIN no recibe acciones sobre OWNER y BARBER/RECEPTIONIST no montan consultas ni acciones administrativas. La UI no presenta teléfono, IDs internos, datos Clerk ni otros campos privados. Las query keys incluyen usuario, organización y rol; cada visita usa una instancia de alcance única, el cambio de contexto desmonta datos, páginas, filtros, formularios y modales, y las respuestas o efectos tardíos solo invalidan su alcance original. Incluye loading, vacío, error/reintento, pending y éxito, tabla desktop, tarjetas móviles y modales con foco, Escape y retorno de foco. No cambia backend, Prisma, Clerk/Auth legacy, Profesionales, Reservas, Facturación, Configuración ni Analytics. Web TypeScript, lint, 41 pruebas y build pasan; QA real anterior sobre Clerk Development y PostgreSQL desechable verificó OWNER/ADMIN/BARBER, paginación, proyección, perfil informativo, protección de OWNER, cambio de tenant y accesibilidad en escritorio. La revisión visual móvil y las mutaciones externas de invitación permanecen para el QA final del propietario antes de aprobación.
