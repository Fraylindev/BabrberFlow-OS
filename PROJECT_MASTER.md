...
- Crear una reserva interna o pública, reprogramar, reactivar una reserva futura cancelada y archivar un Professional usan el mismo bloqueo de fila PostgreSQL `SELECT ... FOR UPDATE`, siempre por `id + organizationId` y dentro de la transacción que realiza la escritura. Si la operación de agenda gana, el archivo observa la reserva futura y devuelve `409`; si el archivo gana, la operación observa `ARCHIVED` y se rechaza. No puede persistir `ARCHIVED` junto con una reserva futura `PENDING`/`CONFIRMED` por una carrera entre esas operaciones.
- La matriz administrativa permite `PENDING → CONFIRMED/CANCELLED`, `CONFIRMED → COMPLETED/NO_SHOW/CANCELLED` y `CANCELLED → PENDING/CONFIRMED`; `COMPLETED` y `NO_SHOW` son terminales. Recuperar una reserva cancelada futura hacia `PENDING` o `CONFIRMED` exige nuevamente Professional `ACTIVE` del mismo tenant; `INACTIVE` y `ARCHIVED` devuelven `409`. Las transiciones limitadas de BARBER definidas en A0 se conservan.
...
- `/auth/invite` respeta `createPublicProfile` únicamente para rol `BARBER`; crea el perfil `ACTIVE` y publicado y permite un perfil por organización sin mezclar tenants. Para un User existente, Membership y Professional automático se escriben en una única transacción: un fallo inesperado del perfil revierte también la Membership y no deja acceso parcial.
