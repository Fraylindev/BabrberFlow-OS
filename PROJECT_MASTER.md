...
| 4 | Servicios — A.1 Backend: ordenamiento | **IMPLEMENTADO / EN REVISIÓN** | `GET /services` amplía `sort`; pendiente de aprobación explícita antes del consumo frontend |
...
Servicios — A.1 Backend de ordenamiento: **IMPLEMENTADO / EN REVISIÓN**. `GET /services` conserva el filtro independiente `isActive` y añade `sort` para nombre, reservas no canceladas, antigüedad y precio. El valor omitido mantiene `name ASC, id ASC`; todos los órdenes tienen desempate estable y permanecen tenant-scoped. Los conteos solo intervienen en el orden, no se exponen, y la respuesta continúa limitada campo por campo a la proyección aprobada. No hay cambio Prisma ni migración. Evidencia candidata: Prisma validate/generate, TypeScript, lint y build en exit `0`; 392 unitarias API aprobadas/11 omitidas; 56/56 unitarias dirigidas y 15/15 E2E aisladas con 19 migraciones. El frontend no puede consumir este contrato hasta la aprobación explícita del propietario.
