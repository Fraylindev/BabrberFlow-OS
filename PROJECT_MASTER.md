...
Servicios — Entrega A Backend queda **CERRADO / APROBADO** por decisión oficial del propietario el 2026-08-31 sobre `f7088a4abb14e61721f08f7eeb85adbd8e6650d6`; A.1 de ordenamiento queda **CERRADO / APROBADO** sobre `9752842dfc8a3a53cdcb6bc06e61a185bc79a385`. Servicios — Entrega B Frontend permanece **IMPLEMENTADO / EN REVISIÓN**: consume las proyecciones mínimas publicadas, filtra y ordena en backend mediante controles independientes, y permite listar, crear, editar, desactivar y reactivar. OWNER/ADMIN gestionan; BARBER/RECEPTIONIST reciben una vista de solo lectura. La consulta se aísla por usuario, organización y rol; cambiar de contexto desmonta datos, filtros y modales, y una instancia única por visita impide efectos tardíos incluso en A → B → A. El formulario conserva el foco al escribir, restringe precio a un monto DOP numérico, expresa duración en minutos enteros con equivalencias naturales y reserva un slot no interactivo para una futura imagen sin integrar Cloudinary ni ampliar el contrato. La UI mantiene loading, vacío, error/reintento, pending y éxito, con tabla desktop y tarjetas móviles. Web TypeScript, lint, 35 pruebas y build pasan. QA real con Clerk Development y PostgreSQL desechable verificó OWNER → BARBER → OWNER, datos y acciones tenant-scoped, combinación de estado/orden, alta y edición sin pérdida de foco, error terminal sin datos obsoletos, reintento, desktop y 375 px sin overflow; consola sin errores de aplicación. No cambia backend, Prisma, Reservas, Facturación, página pública, `ProfessionalService` ni medios.
