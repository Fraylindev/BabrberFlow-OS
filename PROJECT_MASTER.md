...
| 3 | Profesionales Frontend general | **IMPLEMENTADO / EN REVISIÓN** | CRUD, permisos, responsive y búsqueda; correctivo de perfil/contexto candidato, sin cierre general |
| 3 | Profesionales: correctivo de perfil propio y aislamiento | **IMPLEMENTADO / EN REVISIÓN** | Publicación autorizada 2026-08-30; gestión propia A1 + teléfono privado, disponibilidad agrupada, menú Facturación y botón de cambio de cuenta auditado |
...
| 5 | Facturación-B Frontend | **IMPLEMENTADO / EN REVISIÓN** | Rango real por fecha local de emisión, estado, responsive y aislamiento de contexto; pendiente de auditoría y aprobación |
...
Correctivo de perfil propio y aislamiento de Profesionales: **IMPLEMENTADO / EN REVISIÓN** por instrucción de publicación del propietario el 2026-08-30, con autorización de publicación desde `27214da509e9a94358924e6a8be6c2eed8fd793f`. La pantalla usa `usuario + organización + rol` para montaje y listado: cambiar contexto elimina datos, formularios, gestión propia, confirmaciones y disponibilidad anteriores e ignora efectos visuales de mutaciones tardías. Una instancia única por visita evita que A → B → A reactive callbacks de la primera A y se invalida al salir; la regresión automática cubre ambos casos. “Gestionar mi perfil” reúne “Editar información” y “Mi disponibilidad”; conserva/restaura la edición pública propia A1 (`name`, `bio`, `avatar`, `specialty`, `experienceYears`) además del teléfono privado. `PATCH /professionals/me` usa exclusivamente identidad y tenant autenticados, valida la lista explícita, normaliza strings, permite `null` en opcionales y rechaza nombre vacío/`null`, estado, publicación, vínculo, IDs y rol. El teléfono solo se devuelve al perfil propio y gestión OWNER/ADMIN, nunca al directorio mínimo, perfiles ajenos BARBER ni rutas públicas. AuditLog permanece sin PII; no cambia Prisma ni hay migración, subida de medios o edición de identidad Clerk. API/web pasaron TypeScript, lint y build; 354 unitarias API (11 omitidas), 27 web y 22/22 E2E con las 19 migraciones en otro clúster PostgreSQL aislado. La revisión del propietario y sus capturas no sustituyen el QA visual pendiente: OWNER → BARBER → OWNER, cambio de organización con modal abierto, perfil/disponibilidad, error/reintento, teclado, consola y desktop/móvil. Se incluye con autorización el botón del propietario “Usar otra cuenta” en `auth/continue/page.tsx`, conservando su lógica y limpiando solo whitespace final. Cinco pruebas nuevas ejecutan el componente con dobles unitarios y comprueban que no envía onboarding, disabled, payload y redirecciones; navegador local verificó redirección sin sesión a login. Logout Clerk autenticado pendiente, sin cambios de configuración ni backend. El fixture sintético OWNER en A/BARBER en B continúa activo y su retiro/reversión temporal queda fuera del checkpoint. El menú común dice “Facturación” y la pantalla BARBER mantiene “Facturación de mis servicios”, sin alterar operaciones financieras. Esta publicación no cierra Profesionales ni Facturación-B, que permanece **IMPLEMENTADO / EN REVISIÓN**.
