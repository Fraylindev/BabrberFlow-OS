...
export default function DashboardHome() {
  ...
  return (
    <div>
          ...
          organization && (
            <div className="flex gap-2">
              ...
              <a href={`/${organization.slug}`} target="_blank" rel="noopener noreferrer">
                <Button variant="secondary" tone="light">Ver página pública ↗</Button>
              </a>
            </div>
          )
        }
      ...
      {canSeeAnalytics && (
        <Card tone="light" className="mb-6 p-6">
          {loading || !analytics ? (
            ...
            <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
              ...
              <TrendStat
                ...
                displayValue={String(analytics.bookings.pending)}
              />
            </div>
          )}
        </Card>
          ...
          <ul className="flex flex-col gap-2">
            {alerts.map((a) => (
              <li key={a.text}>
                <Link
                  href={a.href}
                  className="flex items-center gap-2 text-sm text-[var(--dash-text)] transition-colors hover:text-[var(--dash-accent)]"
                >
                  <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--dash-accent)]" />
                  {a.text}
                </Link>
              </li>
            ))}
              ...
              {loading ? (
                ...
                <ul className="flex flex-col divide-y divide-[var(--dash-border)]">
                  {todayBookings.map((b) => (
                    <li
                      key={b.id}
                      className={`flex items-center justify-between gap-4 py-3 pl-3 transition-colors duration-150 hover:bg-[var(--dash-surface-raised)] ${
                  ...
                  <ul className="flex flex-col gap-3">
                    {workload.map((w) => (
                      <li key={w.professional.id} className="flex items-center gap-3">
                        ...
                        <span className="font-[family-name:var(--font-mono)] text-sm text-[var(--dash-text-muted)]">
                          {w.count}
                        </span>
                      </li>
                    ))}
