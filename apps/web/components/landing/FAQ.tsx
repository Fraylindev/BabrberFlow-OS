        ...
        <div className="flex flex-col gap-3">
          {FAQS.map((item, i) => (
            <Reveal key={item.q} delay={i * 50}>
              <details className="group rounded-sm border border-[var(--color-border)] bg-[var(--color-surface)] px-5 py-4 transition-colors duration-[var(--duration-base)] open:border-[var(--color-brass)]/40">
                <summary className="flex cursor-pointer list-none items-center justify-between text-sm text-[var(--color-paper)]">
                  {item.q}
                  <span
                    aria-hidden
                    className="ml-4 text-[var(--color-muted)] transition-transform duration-[var(--duration-base)] ease-[var(--ease-out)] group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
